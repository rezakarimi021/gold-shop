...
export const HeroSection = () => {
  ...
  // Fade starts subtle (always present to kill the hard edge),
  // then strengthens as the hero scrolls out of view.
  const fadeOpacity = useTransform(scrollYProgress, [0, 0.55], [0.55, 1]);
  // The overlay also gently rises to meet the content, making the wipe feel dimensional.
  const fadeTranslateY = useTransform(scrollYProgress, [0, 0.55], ["0%", "-6%"]);
  ...
  return (
    ...
    >
      ...
      {/* ── Scroll-fade dissolve ─────────────────────────────────────────────── */}
      {/* Fades the hero into the page background with no hard edge.
        Always visible at reduced opacity to kill the sharp line at rest;
        strengthens as the section scrolls out of view. */}
      <motion.div
        aria-hidden
